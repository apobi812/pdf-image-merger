...
- 언어는 별도 HTML을 만들지 않고 `?lang=en`, `?lang=ja` 같은 URL 파라미터와 상단 국기 버튼으로 전환합니다.
- `sitemap.xml`과 런타임 `hreflang` alternate 링크로 10개 언어 URL을 검색엔진에 명시합니다.
...
- Worker는 허용된 이벤트명, 도구, 경로, 언어만 저장하고 관리자 경로 분석 payload와 과대 JSON을 거절합니다.
- 파일 형식과 크기를 클라이언트에서 제한합니다.
- PDF/이미지 병합 입력은 개별 파일 크기와 전체 입력 크기를 함께 제한해 브라우저 메모리 고갈을 줄입니다.
- 글자수 세기는 과도하게 큰 붙여넣기로 인한 브라우저 멈춤을 줄이기 위해 최대 입력 글자수를 제한합니다.
- PDF와 이미지 파일은 실제 파일 시그니처를 확인하고, SVG 등 병합 대상이 아닌 형식은 거절합니다.
- 이미지는 PDF에 넣기 전에 브라우저에서 PNG로 재인코딩해 EXIF/위치정보 같은 원본 메타데이터가 따라가지 않게 합니다.
...
- `worker/`의 Cloudflare Worker + D1 백엔드를 연결하면 PBKDF2 기반 서버형 관리자 로그인과 전체 사용자 집계가 활성화됩니다.
...
`config.js`를 바꾼 뒤에는 `npm run metadata`를 실행해 `sitemap.xml`, `robots.txt`, 정적 canonical 태그를 같은 도메인으로 재생성합니다.
...
전체 시스템 구조와 완료 증거는 `docs/ARCHITECTURE.md`를 기준으로 확인합니다.
